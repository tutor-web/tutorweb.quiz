...
module.exports = function Quiz(rawLocalStorage, ajaxApi) {
    ...
    this.getAvailableLectures = function (onSuccess) {
        ...
        function lecToObject(l) {
            return {
                "uri": l.uri,
                "title": l.title,
                "grade": self.gradeString(arrayLast(l.answerQueue)),
                "synced": isSynced(l)
            };
        }
        for (k in twIndex) {
    ...
    this.setQuestionAnswer = function (formData, onSuccess) {
        ...
        self._getQuestionData(a.uri, true).then(function (qn) {
            ...
            if (a.question_type === 'template') {
                ...
                if (!qn.student_answer) { // If we're re-working a question, shouldn't get graded for it.
                    a.correct = !!a.student_answer.text;
                    if (!a.correct) {
    ...
    this.syncTutorialQuestions = function (tutUri) {
        ...
        return curTutorial.lectures.map(function (l) {
            ...
            // Turn into AJAX promise
            return ajaxApi.ajax(c);
        });
    };
