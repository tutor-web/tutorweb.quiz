   ...
   /** Update sync button, curState one of 'processing', 'online', 'offline', 'unauth', '' */
    this.syncState = function (curState) {
        ...
        if (!curState) {
            ...
            return jqSync[0].className === 'button button-sm active' ? 'processing'
                    : jqSync[0].className === 'button button-sm button-danger btn-unauth' ? 'unauth'
                    : jqSync[0].className === 'button button-sm button-success' ? 'online'
                         : 'unknown';
        }

        // Setting the state
        if (curState === 'processing') {
            jqSync[0].className = 'button button-sm active';
            jqSync.text("Syncing...");
        } else if (curState === 'online') {
            jqSync[0].className = 'button button-sm button-success';
            ...
            jqSync[0].className = 'button button-sm button-danger';
            jqSync.text("Syncing failed!");
        } else {
            jqSync[0].className = 'button button-sm';
        ...
        case 'initial':
            ...
            quiz.setCurrentLecture(quiz.parseQS(window.location), function (a, continuing) {
                ...
                if (continuing == 'practice') {
                    ...
                    twView.updateActions(['quiz-real', 'gohome', 'quiz-practice']);
                }
            });
            break;
        case 'quiz-real':
