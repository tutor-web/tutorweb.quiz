...
(function (window, $) {
    ...
    twView.states['qn-skip'] = twView.states['qn-submit'] = twView.states['ug-skip'] = twView.states['ug-submit'] = twView.states['ug-rate'] = function (curState) {
        ...
        return quiz.setQuestionAnswer(curState.endsWith('-skip') ? [] : twView.jqQuiz.children('form').serializeArray()).then(function (args) {
            ...
            if (args.a.question_type === 'usergenerated' && !args.a.student_answer.hasOwnProperty('comments')) {
                ...
                } else if (args.nextLecture) {
                    twView.nextLectureHref = twView.generateUrl({lecUri: args.nextLecture});
                    twView.updateActions(['gohome', 'review', 'quiz-real', 'go-next']);
                } else {
                    twView.updateActions(['gohome', 'review', (args.practiceAllowed > 0 ? 'quiz-practice' : null), 'quiz-real']);
                }
            }
    ...
    twView.states['go-next'] = function () {
        window.location.href = 'quiz.html' + twView.nextLectureHref;
    };
    ...
    twView.states.logout = function () {
        // TODO: Should be querying lecture state here, but for error states anyway.
        quiz.clearStorage();
        if (window.location.pathname.indexOf("++resource++tutorweb.quiz") > -1) {
            window.location.href = twView.ajaxApi.ploneUrl('/logout');
            return;
        }
        return twView.ajaxApi.logout().then(function () {
