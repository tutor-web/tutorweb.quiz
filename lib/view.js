/*jslint nomen: true, plusplus: true, browser:true, regexp: true, todo: true, unparam: true */
/*global module, Promise, URL */
...
module.exports = function View($) {
    ...
    this.renderQuestion = function (qn, a) {
    ...
    this.states['go-login'] = function (curState, updateState) {
        var creds, jqForm = twView.jqQuiz.children("form.login-form");

        if (window.location.pathname.indexOf("++resource++tutorweb.quiz") > -1) {
            // Use cookie based login when hosted in Plone site
            return twView.ajaxApi.getHtml(twView.ajaxApi.ploneUrl('/login?came_from=' + encodeURIComponent(window.document.location))).then(function (docString) {
        ...
        twView.updateActions(['go-register', 'go-forgot-pass', 'go-login']);

        if (jqForm.length > 0) {
            // Form already present, try to login
            creds = twView.formAsDict(jqForm);

            return twView.ajaxApi.login(creds.login, creds.password).then(function () {
                return 'reload';
            })['catch'](function () {
                twView.showAlert('error', 'Login failed, please check your username and password.');
            });
        }

        jqForm = el('form').attr('class', 'login-form').append([
            el('label').text('Username'),
            el('input').attr('type', 'text').attr('name', 'login').attr('required', 'required'),
            el('label').text('Password'),
            el('input').attr('type', 'password').attr('name', 'password').attr('required', 'required'),
            el('input').attr('type', 'submit').css('display', 'none'),
        ]);

        jqForm.on('submit', function (e) {
            e.stopPropagation();
            e.preventDefault();
            updateState('go-login');
        });

        twView.jqQuiz.empty().append(jqForm);
    };

    this.states['go-register'] = function () {
        window.location.href = this.ajaxApi.ploneUrl("/@@register?came_from=" + encodeURIComponent((new URL("start.html", window.location)).toString()));
    };

    this.states['go-forgot-pass'] = function () {
        var jqForm = twView.jqQuiz.children("form.login-form"),
            creds = jqForm.length > 0 ? twView.formAsDict(jqForm) : {};

        window.location.href = this.ajaxApi.ploneUrl("/mail_password_form?userid=" + encodeURIComponent(creds.login));
    };
    ...
    this.stateMachine = function (updateState) {
        ...
        function fallback(curState) {
            ...
            default:
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    url: self.ajaxApi.ploneUrl('/@@quizdb-logerror'),
                    timeout: 1000,
                });
        ...
        window.onerror = function (message, url, linenumber) {
            ...
            } else if (message.indexOf('tutorweb::unauth::') > -1 && message.indexOf('drill is for user') > -1) {
                parts = message.split(/\:\:/);
                newState = 'error-wronguser';
