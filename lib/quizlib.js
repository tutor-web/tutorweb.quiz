...
module.exports = function Quiz(rawLocalStorage, ajaxApi) {
    ...
    this.getNewQuestion = function (opts, onSuccess) {
        ...
        Promise.resolve().then(function assignNewQuestion(attempts) {
            ...
            // Assign new question
            a = iaalib.newAllocation(self.curTutorial, self.lecIndex, answerQueue, opts);
    ...
    this.syncQuestions = function () {
        ...
        if (missingQns.length >= Math.min(10, curLecture.questions.length)) {
            ...
            return [ajaxApi.getJson(curLecture.question_uri).then(function (data) {
    ...
    this.gradeSummary = function (lecture) {
        ...
        if (currentGrade >= 9.750) {
            ...
            out.encouragement = "Win " + lecture.settings.award_lecture_aced + " SMLY if you ace this lecture, bonus " + lecture.settings.award_tutorial_aced + " SMLY for acing whole tutorial";
        }
