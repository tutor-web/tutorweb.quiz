...
function ChatView($) {
    ...
    this.startTimer = function (remainingSeconds, maxSeconds) {
        var freeSeconds = Math.max(60 - maxSeconds + remainingSeconds, 0);

        // First minute is free
        if (freeSeconds > 0 && !this.freeSecondsTimer) {
            this.addMessage('system', 'If you disconnect in ' + Math.round(freeSeconds) + ' seconds, this session will be free.');
            this.freeSecondsTimer = window.setTimeout(function () {
                self.addMessage('system', 'Free time used up');
            }, freeSeconds * 1000);
        }
    ...
    this.addMessage = function (author, message) {
        ...
        //TODO: Window title notification of new messages
