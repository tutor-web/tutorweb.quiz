...
function renderGgb(jqEl) {
    ...
    return Promise.all(jqEl.find('.geogebra_applet').toArray().map(function (el) {
        var applet;
        ...
        ggbCount++;
        el.insertAdjacentHTML('beforebegin', '<div class="geogebra_applet" id="applet' + ggbCount + '"></div>');
        applet = new window.GGBApplet({
            filename: el.getAttribute('href'),
            showToolbar: true,
        }, true);
        applet.inject('applet' + ggbCount);
        return new Promise(function (resolve) {
            setTimeout(function () {
                resolve(window.ggbApplet.setWidth(el.parentElement.offsetWidth));
            }, 5000);
        });
    }));
}
...
module.exports.renderTex = function ($, jqEl) {
    ...
    return rt.renderTex(jqEl).then(renderGgb).then(function () {
