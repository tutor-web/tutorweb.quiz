...
module.exports = function Quiz(rawLocalStorage, ajaxApi) {
    ...
    this.subscribeToLecture = function () {
        //TODO: We want to tell the server that we want to get at "something"
        // based on URI.
        return true;
    };
    ...
    this.getAvailableLectures = function () {
        ...
        return Promise.resolve().then(function () {
            ...
            return Promise.all(lectureUrisFromSubscription(subscriptions).map(function (uri) {
                return self._getLecture(uri).then(function (l) {
                    var gradeString, gradeSummary = self._gradeSummary(l);
    ...
    this._getSubscriptions = function () {
        var self = this,
    ...
    this._getLecture = function (lecUri) {
        var self = this;
    ...
    this._withLecture = function (lecUri, work) {
        var self = this, lec;

        return self._getLecture(lecUri).then(function (lecture) {
            lec = lecture;  // Store it in our function scope
            return Promise.resolve(lecture);
        }).then(work).then(function (rv) {
            //TODO: Hide lec.uri from storage?
            self.ls.setItem(lec.uri, lec);
            return rv;
        });
    };
    ...
    this.setCurrentLecture = function (params) {
        ...
        return self._getLecture().then(function (lecture) {
            ...
            return {
                ...
                lecUri: lecture.uri,
                lecTitle: lecture.title, //TODO: Does the lecture have a title? Does it have the tutorial title too?
            };
        });
    };
    ...
    this.insertTutorial = function (tutId, tutTitle, lectures, questions) {
        ...
        if (!self.ls.getItem('_subscriptions')) {
            self.ls.setItem('_subscriptions', {children: []});
        }
    ...
    function _queueMerge(preQ, currentQ, serverQ) {
        ...
        function runningTotal(a, prop, extra) {
            if (totals.hasOwnProperty(prop)) {
                ...
                // First entry, so believe the entry
                totals[prop] = a[prop] || 0;
            }
            return totals[prop];
        }
    ...
    this.syncSubscriptions = function (opts, progressFn) {
        ...
        return self.ajaxApi.getJson('/@@quizdb-subscriptions').then(function (subscriptions) {
            self.ls.setItem('_subscriptions', subscriptions);
            return subscriptions;
        }).then(function (subscriptions) {
            var lectureUris = lectureUrisFromSubscription(subscriptions),
                opSucceeded = Object.keys(lectureUris).map(function () { return 0; }),
                opTotal = Object.keys(lectureUris).map(function () { return 0; });

            return Promise.all(lectureUris.map(function (uri, i) {
                return self.syncLecture(uri, {
                    ...
                    opSucceeded[i] = lecSucceeded;
                    opTotal[i] = lecTotal;
                    progressFn(sum(opTotal), sum(opSucceeded), "Lecture " + (i + 1) + ": " + message);
                });
            })).then(function () {
                opTotal = sum(opTotal);  // NB: Don't need the array any more
    ...
    this.syncLecture = function (lecUri, opts, progressFn) {
        ...
        return self._getLecture(lecUri)['catch'](function (err) {
            var lec;

            if (err.message.indexOf("Unknown lecture: ") === 0) {
                if (opts.ifMissing === 'fetch') {
                    // Insert dummy item for us to then update
                    lec = {
                        uri: lecUri || self.lecUri,
                        answerQueue: [],
                    };
                    self.ls.setItem(lec.uri, lec);
                    return lec;
            ...
            return self.ajaxApi.postJson(preSyncLecture.uri, preSyncLecture).then(function (newLecture) {
