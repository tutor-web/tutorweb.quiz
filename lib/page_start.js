    ...
    this.renderChooseLecture = function (subscriptions, unselectedActions, selectedActions) {
        ...
        function toSelectList(lecInfo, s) {
            var jqEl = el('a').text(s.title || ''),
                info = lecInfo[s.uri || ''] || {},
                out = (s.children || []).map(toSelectList.bind(null, lecInfo));
        ...
        self.jqQuiz.empty().append([
            self.selectList(
                toSelectList(subscriptions.lectures, subscriptions.subscriptions),
                unselectedActions,
                selectedActions
            ),
...
(function (window, $) {
    ...
    twView.states['sync-subscriptions'] = twView.states['sync-subscriptions-force'] = function (curState) {
        return quiz.syncSubscriptions({ syncForce: curState === 'sync-tutorials-force' }, function (opTotal, opSucceeded, message) {
    ...
    twView.states.lecturemenu = function () {
        return quiz.getAvailableLectures().then(function (subscriptions) {
            if (subscriptions.lectures.length === 0) {
