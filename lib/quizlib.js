...
module.exports = function Quiz(rawLocalStorage, ajaxApi) {
    ...
    this.syncSubscriptions = function (opts, progressFn) {
        ...
        function sum(obj) {
            return Object.keys(obj).reduce(function (a, b) { return a + obj[b]; }, 0);
        }
        ...
        return self.ajaxApi.postJson('/@@quizdb-subscriptions', postData).then(function (subscriptions) {
            ...
            var i, p = Promise.resolve(),
                lectureUris = lectureUrisFromSubscription(subscriptions),
                opSucceeded = {},
                opTotal = {};

            function syncLectureFn(uri) {
                return function () {
                    return self.syncLecture(uri, {
                        ...
                        opSucceeded[uri] = lecSucceeded;
                        opTotal[uri] = lecTotal;
                        progressFn(sum(opTotal), sum(opSucceeded), uri + ": " + message);
                    });
                };
            }
            // Sync each lecture in order (lots just causes them to back up on the server)
            for (i = 0; i < lectureUris.length; i++) {
                opTotal[lectureUris[i]] = 3;
                p = p.then(syncLectureFn(lectureUris[i]));
            }
    ...
    this.syncLecture = function (lecUri, opts, progressFn) {
        ...
        return self._getLecture(lecUri, opts.ifMissing === 'fetch').then(function (preSyncLecture) {
            ...
            return self.ajaxApi.postJson(preSyncLecture.uri, preSyncLecture, { timeout: 10 * 1000 }).then(function (newLecture) {
