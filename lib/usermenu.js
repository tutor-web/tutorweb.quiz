...
module.exports = function UserMenu(jqUserMenu, quiz) {
    ...
    this.transitions = {
        'initial': function () {
            return Promise.resolve(window.applicationCache ? 'appcache-check' : 'connect');
        },
        'connect': function () {
            renderMenu({
                text: "Connecting...",
            ...
            return quiz.updateUserDetails('//' + window.document.location.host + '/', null).then(function (user) {
                ...
                if (quiz.isLectureSelected()) {
                    menu.action.unshift(
                        { text: "Sync with server", action: "menustate:sync-force" },
                        { text: "Get some help on this lecture", action: "popup:chat.html#!lecUri=" + encodeURIComponent(quiz.getCurrentLecture().uri) }
                    );
                } else if (document.location.pathname.match(/start.html$/)) {
                    menu.action.unshift(
                        { text: "Sync tutorials", action: "twstate:sync-tutorials-force" },
        ...
        'appcache-check': function () {
            return new Promise(function (resolve) {
                ...
                ac.addEventListener('error', function (e) {
                    resolve('appcache-error');
                }, false);
                ...
                ac.addEventListener('cached', function () {
                    self.updateState('appcache-finish');
                    resolve('connect'); // First time, so no need to reload
                }, false);
                ac.addEventListener('updateready', function () {
                    self.updateState('appcache-reload');
                    resolve('appcache-reload');
                }, false);
