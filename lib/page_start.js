...
(function (window, $) {
    ...
    twView.states['sync-subscriptions'] = twView.states['subscription-remove'] = twView.states['sync-subscriptions-force'] = function (curState) {
        return quiz.syncSubscriptions({
            syncForce: curState === 'sync-subscriptions-force',
        ...
        return quiz.getAvailableLectures().then(function (subscriptions) {
            if (Object.keys(subscriptions.lectures).length === 0) {
                ...
                twView.renderChooseLecture(
                    subscriptions,
                    [],
                    ['subscription-remove', 'go-slides', 'go-drill']
                );
            }
