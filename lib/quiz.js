...
function QuizView($) {
    ...
    this.renderNewQuestion = function (qn, a, actionsOnChange, onFinish) {
        ...
        if (qn._type === 'template') {
            ...
            jqForm.append([
                el('h3').text(qn.title),
                el('p').html(qn.hints),
                self.previewTeX(el('textarea').attr('name', 'text').attr('placeholder', qn.example_text).text(qn.student_answer.text)),
                el('label').text("Write the correct answer below"),
                self.previewTeX(el('input').attr('type', 'text')
                                      .attr('name', 'choice_' + '0')
                ...
                el('div').append(qn.example_choices.slice(1).map(function (text, i) {
                    return self.previewTeX(el('input').attr('type', 'text')
                ...
                self.previewTeX(el('textarea').attr('name', 'explanation').attr('placeholder', qn.example_explanation).text(qn.student_answer.explanation))
            ]);
        } else {
            jqForm.append([
    ...
    twView.stateMachine(function updateState(curState, fallback) {
        ...
        case 'set-lecture':
            ...
            quiz.setCurrentLecture(twView.parseQS(window.location), function (a, continuing) {
                ...
                twView.renderGradeSummary(quiz.gradeSummary());
        ...
        case 'rewrite-question':
            ...
            quiz.getNewQuestion({
                ...
                var actions = args.qn._type === 'template' ? ['ug-skip', 'ug-submit'] : ['qn-skip', 'qn-submit'];

                twView.renderGradeSummary(quiz.gradeSummary());
                twView.renderNewQuestion(args.qn, args.a, actions, function () {
                    twView.updateActions([actions[0]]);
        ...
        case 'ug-rate':
            ...
            twView.updateActions([]);
            quiz.setQuestionAnswer(curState.endsWith('-skip') ? [] : twView.jqQuiz.children('form').serializeArray()).then(function (args) {
