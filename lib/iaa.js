...
module.exports = function IAA() {
    ...
    this.newAllocation = function (curLec, opts) {
        ...
        if (opts.question_uri) {
            qn = findQn(questions, opts.question_uri);
        } else if (Math.random() < getSetting(settings, 'hist_sel', 0)) {
            // Choose one of the historical questions, but completely randomly
            qn = this.iaaAdaptive(questions, answerQueue, settings, opts, 'historical');
        ...
        if (!qn) {
            throw new Error("tutorweb::noquestions::No suitable questions in lecture");
        }
        return {
