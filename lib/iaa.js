...
module.exports = function IAA() {
    ...
    /**
      ...
      */
    this.newAllocation = function (curTutorial, lecIndex, answerQueue, opts) {
        var questions, qn,
        ...
        if (opts.question_uri) {
            qn = findQn(curTutorial.lectures[lecIndex].questions, opts.question_uri);
        } else {
            // Choose question bank based on hist_sel
            if (Math.random() < getSetting(settings, 'hist_sel', 0)) {
            ...
            switch (getSetting(settings, 'iaa_type', 'adaptive')) {
            case 'exam':
                qn = this.iaaExam(questions, answerQueue, settings, opts);
                break;
            case 'adaptive':
                qn = this.iaaAdaptive(questions, answerQueue, settings, opts);
                break;
            default:
    ...
    this.iaaAdaptive = function (questions, answerQueue, settings, opts) {
    ...
    this.gradeAllocation = function (settings, answerQueue, lecture) {
        ...
        algorithms.weighting = function (aq) {
        ...
        algorithms.ratiocorrect = function (aq) {
            ...
            return qns_correct / lecture.questions.length;
        };

        // Choose algorithm
        grade = algorithms[getSetting(settings, 'grade_algorithm', 'weighting')];
    ...
    this.chooseQuestion = function (qnDistribution) {
        ...
        if (qnDistribution.length === 0) {
            throw new Error("Lecture has no questions of selected type");
        }
