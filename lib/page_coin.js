    ...
    this.renderCoinAccount = function (data) {
        ...
        if (data.coin_available <= 0) {
            ...
            self.jqQuiz.append([
                ...
                el('p').html('Unused SMLY will be donated to the Education in a Suitcase project. You can also do this now by pressing the donate button below. 8 million SMLY will buy one tablet computer for a student in Kenya.'),
...
(function (window, $) {
    ...
    twView.stateMachine(function updateState(curState, fallback) {
        ...
        case 'view-award':
            walletAddr = curState === 'coinaward-donate' ? 'BPj18BBacYdvEnqgJqKVRNFQrw5ka76gxy' :
                         curState === 'coinaward-redeem' ? twView.getWalletId() :
                            null;
            quiz.updateAward(twView.portalRootUrl(), walletAddr).then(function (data) {
                if (data.coin_available > 0) {
