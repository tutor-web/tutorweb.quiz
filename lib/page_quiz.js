...
(function (window, $) {
    ...
    twView.states['set-lecture'] = function () {
        ...
        return quiz.setCurrentLecture({lecUri: twView.curUrl.lecUri}).then(function (args) {
            ...
            if (err.message.indexOf("Unknown lecture: ") === 0) {
                twView.showAlert('info', 'You are not subscribed to this lecture yet, you need to subscribe before taking drills. Do you wish to?');
                twView.updateActions(['subscribe-lecture']);
                return;
            }
    ...
    twView.states['subscribe-lecture'] = function () {
        twView.updateActions([]);
