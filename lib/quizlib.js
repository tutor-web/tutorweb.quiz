...
module.exports = function Quiz(rawLocalStorage, ajaxApi) {
    ...
    this.getNewQuestion = function (opts) {
        ...
        return self._withLecture(null, function (curLecture) {
            return new Promise(function (resolve) {
                if (!window.twAds || !(Math.random() < getSetting(curLecture.settings, 'ad_prob', 0.3))) {
                    // Not built with ads / not time to show ad
                    resolve(null);
                } else {
                    window.twAds.showInterstitial().then(resolve);
                }
            }).then(function (adSuccess) {
                // NB: adSuccess true - ad viewed, false - ad failed/blocked, null - not showing ad this time
                return adSuccess;
            }).then(function () { return tryRepeatedly(function () {  // Repeatedly try assigning a new question, until one works
    ...
    this._getQuestionData = function (curLecture, uri, cachedOkay) {
        ...
        if (self._lastFetched && self._lastFetched.all_questions && self._lastFetched.question_uri === curLecture.question_uri) {
        ...
        p = p.then(function (all_qns) {
            // Not sure why, but some students have a null all_questions, try and recover.
            // Possibly a Safari bug?
            if (all_qns) {
                return all_qns;
            }
        ...
        return p.then(function (all_qns) {
            if (!all_qns) {
                throw new Error("No questions fetched for " + self._lastFetched.question_uri);
            }
            if (all_qns[uri]) {
    ...
    this.removeUnusedObjects = function () {
        ...
        self.ls.listItems().map(function (k) {
            // NB: Keep token set in ajaxApi
            lsContent[k] = k === 'tw-jwt-token' ? 1 : 0;
        });
