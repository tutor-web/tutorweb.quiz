/*jslint nomen: true, plusplus: true, browser:true, todo: true, unparam: true, regexp: true*/
/*global module */
...
module.exports = function IAA() {
    ...
    this.iaaExam = function (questions, answerQueue, settings, opts) {
        ...
        if (answerQueue.length >= questions.length) {
            throw new Error("tutorweb::noquestions::" + getSetting(settings, 'iaa_exam_finished_tmpl', "You have answered all questions. Press 'Back to main menu' to choose another lecture").replace(/\$\{([a-z._]+)\}/g, function (full_match, prop) {
                switch (prop) {
                case 'final_grade':
                    return arrayLast(answerQueue).grade_after;
                case 'maximum_grade':
                    return 10;  // NB: Both weighted and ratiocorrect do 0..10
                case 'failed_topics':
                    return Object.keys(answerQueue.reduce(function (acc, a, i) {
                        if (a.correct === false && questions[i] && questions[i].uri.indexOf('?title=') > '-1') {
                            // Munge question URI into topic, add to dict
                            acc[decodeURIComponent(questions[i].uri.replace(/^.+?title\=/, ''))] = true;
                        }
    ...
    this.questionStudyTime = function (settings, aq) {
        ...
        var rv = getSetting(settings, 'studytime_factor', 2) * incorrectInARow(aq) +
            getSetting(settings, 'studytime_answeredfactor', 0) * (arrayLast(aq, {}).lec_answered || 0);
        rv = Math.min(rv, getSetting(settings, 'studytime_max', 20));
        rv = Math.max(rv, getSetting(settings, 'studytime_baseline', 0));
        return rv;
    };
